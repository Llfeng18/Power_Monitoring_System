...
void ILI9341_Init ( void )
{
  ...
  ILI9341_Clear(0,0,LCD_X_LENGTH,LCD_Y_LENGTH);	/* 清屏，显示全黑 */
	/********显示字符串示例*******/
  ILI9341_DispStringLine_EN(LINE(3),"  system");
	ILI9341_DispStringLine_EN(LINE(5),"   init");
...
void LCD_ERROR_RESET(uint32_t Baud_rate)
{
	uint16_t i;
	uint8_t str[10]; 
	...
	sprintf((char *)str,"Baud rate %d",Baud_rate);
	ILI9341_DispStringLine_EN(LINE(6),(char*)str);
	ILI9341_DispStringLine_EN(LINE(8),"Meter conflicts");
	
	ILI9341_DispStringLine_EN(LINE(10),"TRY RESET");
