...
{	
 ...
 /**
  * @brief  HPLC USART GPIO 配置,工作参数配置
  * @param  无
  * @retval 无
  */
void HPLC_USART_Config(void)
{
  ...
  // 将USART Rx的GPIO配置为浮空输入模式
	...
	USART_ITConfig ( HPLC_USART1, USART_IT_IDLE, ENABLE ); //使能串口总线空闲中断 	
...
void Pi_USART_Config(void)
{
  ...
  // 将USART Rx的GPIO配置为浮空输入模式
	...
	USART_ITConfig ( Pi_USART4, USART_IT_IDLE, ENABLE ); //使能串口总线空闲中断 	
...
///重定向c库函数printf到HPLC串口，重定向后可使用printf函数
...
///重定向c库函数scanf到HPLC串口，重写向后可使用scanf、getchar等函数
int fgetc(FILE *f)
{
		/* 等待串口输入数据 */
